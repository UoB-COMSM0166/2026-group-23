// ============================================================
//  monsters/mobs/ghostbird.js — GhostBird
...
class GhostBird extends Monster {
  constructor(path) {
    ...
    this.baseSpd = 2.2;
    this.ghostTimer = 0;       // Cooldown until next cloak
    this.ghostCooldown = 300;  // Cloak triggers every 5 seconds
    this.isGhost = false;      // Whether currently cloaked
    this.ghostDur = 90;        // Cloak lasts 1.5 seconds
    this.ghostFrames = 0;
    this.ghostPulse = 0;
    this.trail = [];
  }
  takeDamage(dmg) {
    if (this.isGhost) return; // Fully immune while cloaked
  ...
  draw() {
    const alpha = this.isGhost ? 55 : 220; // Semi-transparent while cloaked
