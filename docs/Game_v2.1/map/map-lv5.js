// ============================================================
//  map/map-lv5.js  -  Level 5 antmech apocalypse - scorched ruins
//  Style: blood-red scorched earth + ruined towers + dead trees + broken ground + machine wreckage
// ============================================================
...
function _bg_ruin(lv, theme) {
  ...
  // -- Scorched ground (dark red-brown with cracks) --
  for (let gx=0; gx<GRID_COLS; gx++) {
    for (let gy=1; gy<GRID_ROWS; gy++) {
      ...
      // Crack pattern (two random short lines)
  ...
  // -- Path base color (dark blood stone) --
  noStroke();
  for (let gx=0; gx<GRID_COLS; gx++)
...
function _deco_ruin() {
  ...
  for (const d of _decoCache) {
    ...
    if (type===0) {        // Ruined tower fragment
      const sz=(16+d.r2*12)*ds;
      // Shadow
      fill(0,0,0,38); ellipse(sz*.2,sz*.62,sz*1.5,sz*.3);
      // Tower body (broken rectangle)
      fill(38+d.r3*15,16+d.r3*8,12+d.r3*6, 250);
      // Random gap feel: built up from multiple rectangles
      rect(-sz*.28,-sz*.75, sz*.56, sz*.88, 2);
      // Irregular top break
      fill(14,4,6,255); // 'Cut' with the background color
      ...
      // Stone masonry pattern (horizontal dark lines)
      fill(22,8,6,120);
      for (let k=0;k<3;k++) {
        const ly=-sz*.5+k*sz*.25;
        rect(-sz*.26,ly, sz*.52, 2);
      }
      // Window holes (black rectangles)
      if (d.r3>0.35) {
        fill(0,0,0,220);
    ...
    } else if (type===1) { // Dead tree (trunk + branches)
      ...
      // Branches (multiple, with broken feel)
    ...
    } else if (type===2) { // Machine wreckage (gear/steel frame)
      const sz=(10+d.r2*9)*ds;
      // Shadow
      fill(0,0,0,32); ellipse(sz*.1,sz*.5,sz*1.5,sz*.3);
      // Gear frame (main ring)
